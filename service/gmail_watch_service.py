...
class GmailWatchService:
    ...
    def get_all_user_emails(self) -> list[str]:
        ...
        if not self.tokens_dir.exists():
            logger.info("GmailWatchService: tokens directory does not exist: %s", self.tokens_dir)
            return emails
        ...
        for token_file in self.tokens_dir.glob("*.json"):
            if not token_file.name.endswith('.state.json'):
                ...
                if '@' in email:  # ตรวจสอบว่าเป็นอีเมล
                    emails.append(email)
        logger.info("GmailWatchService: discovered %d token(s) in %s", len(emails), self.tokens_dir)
        return emails
    ...
    def save_watch_state(self, email: str, watch_response: Dict) -> None:
        ...
        try:
            ...
            logger.info("GmailWatchService: state saved for %s (historyId=%s, expiration=%s)", email, watch_response.get('historyId'), watch_response.get('expiration'))
        except Exception:
    ...
    def setup_watch(self, email: str) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.info("Confirmed Gmail profile for %s (messagesTotal=%s, threadsTotal=%s)", profile.get("emailAddress"), profile.get("messagesTotal"), profile.get("threadsTotal"))
            except Exception:
                pass
            return True
    ...
    def is_watch_expired(self, email: str) -> bool:
        ...
        if not state:
            logger.info("GmailWatchService: no existing watch state for %s", email)
            return True
        ...
        if not expiration:
            ...
            logger.info("GmailWatchService: missing expiration in state for %s", email)
            return True
        ...
        try:
            ...
            logger.info("GmailWatchService: time to expiry for %s is %.0f seconds", email, remaining)
        ...
        for email in emails:
            try:
                if self.is_watch_expired(email):
                    ...
                    logger.info(f"Watch for {email} is still valid")
